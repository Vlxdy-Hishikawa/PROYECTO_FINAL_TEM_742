...
<html lang="es">
    ...
    <body>     
        ...
        <c:if test="${hoja.id_seguimiento == 0}">
            <h1 align="center">REGISTRAR DATOS DEL SEGUIMIENTO</h1>
        </c:if>
        <c:if test="${hoja.id_seguimiento != 0}">
                <h1 align="center">MODIFICAR DATOS DEL SEGUIMIENTO  </h1>
        </c:if>
            <form action="SeguimientoControlador" method="post" enctype="multipart/form-data">
                <h3 align="center">
                <c:if test="${hoja.id_seguimiento == 0}">
                    INGRESE LOS DATOS DEL SEGUIMIENTO
                </c:if>
                <c:if test="${hoja.id_seguimiento != 0}">
                    MODIFIQUE LOS DATOS DEL SEGUIMIENTO
                </c:if> 
            </h3>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <select name="estado">
                            <option value=""         
                                    <c:if test="${hoja.estado== ''}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >Seleccione una opcion</option>
                            <option value="INICIADO" 
                                    <c:if test="${hoja.estado == 'INICIADO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >INICIADO</option>
                            <option value="ACTIVO"         
                                    <c:if test="${hoja.estado == 'ACTIVO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >ACTIVO</option>
                            <option value="INACTIVO" 
                                    <c:if test="${hoja.estado == 'INACTIVO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >INACTIVO</option>
                            <option value="EN PROCESO" 
                                    <c:if test="${hoja.estado == 'EN PROCESO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >EN PROCESO</option>
                            <option value="PENDIENTE" 
                                    <c:if test="${hoja.estado == 'PENDIENTE'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >PENDIENTE</option>
                            <option value="FINALIZADO" 
                                    <c:if test="${hoja.estado == 'FINALIZADO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >FINALIZADO</option>
                            <option value="ARCHIVADO" 
                                    <c:if test="${hoja.estado == 'ARCHIVADO'}" var="res" scope="request">
                                        selected
                                    </c:if>
                                    >ARCHIVADO</option>
                        </select>
