 ...
 * @author Usuario
 */
@WebServlet(name = "HojaControlador", urlPatterns = {"/HojaControlador"})
@MultipartConfig(
        ...
        try {
            ...
            switch (action) {
                case "add":
                    ...
                    request.getRequestDispatcher("FormHoja.jsp").forward(request, response);
                ...
                case "view":
                    ...
                    id = Integer.parseInt(request.getParameter("id"));
                    List<HojaRuta> hoj = dao.getByus(id);

                    request.setAttribute("hoj", hoj);
                    request.getRequestDispatcher("Hoja.jsp").forward(request, response);
                    break;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id_hoja_ruta == 0) {
            try {
                HojaRutaDAO dao = new HojaRutaDAOoperations();
                dao.insert(hoja);
                response.sendRedirect(request.getContextPath() + "/HojaControlador?action=view&id="+aux);
